/**
 * Worker-side half of `event-loop.test.ts` phase C.
 *
 * Bun workers are threads of the SAME process, so the dlopen'ed
 * library and the event-loop queue are shared with the main thread.
 *
 * Blocking is intentional and safe here: the worker has already
 * reported "ready", `loop_run()` only returns once the main thread
 * calls `loop_stop()` (which the test does before terminating the
