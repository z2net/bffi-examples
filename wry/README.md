# wry - a real webview window driven from Bun
...
marshal-and-wait (`bffi::invoke_wait`) - which serves BOTH callback
tables since Phase 3.5: a JS-bound handle (`bffi_callback_bind`)
is invoked directly, no adapter glue needed.
...
```
 ...
 webview_bind_ipc(handle, ipc)       ipc_handler wired HERE
  (ipc = the JS-bound callback       .
   handle from bffi_callback_bind)   .
 webview_eval(handle, js) --Eval---> evaluate_script
                                      .
                           page click: window.ipc.postMessage(JSON)
                                      .
                                      ipc handler (loop thread):
                                      invoke_wait(ipc, [Value::Str(body)], 30s)
                                      - the loop thread PARKS -
 webview_ipc_reply(handle, reply)    [JS thread, during pump]:
        ^                                the marshal job converts the
        |                                argument to a cstring and calls
        |                                the bound JSCallback pointer
        |                                synchronously; the JS handler
        |                                computes the reply and stores
        |                                it in the box
                                     the loop thread wakes, reads
...
- **The JS handler is a JS-BOUND callback** (`bffi_callback_bind`,
  the `callbacks` pattern): the JS side wraps its handler
  into a `bun:ffi` JSCallback (`{ args: ["cstring"], returns:
  "void" }`), binds it with the wire signature `unit(str)`
  (`[TAG_UNIT, TAG_STR]`) and hands the HANDLE to
  `webview_bind_ipc`. Since Phase 3.5 `invoke_wait` dispatches such
  handles directly: the marshal job converts the `Value::Str`
  argument to a `cstring` and calls the pointer on the JS thread -
  no forwarder callback, no raw-pointer plumbing on the native
  side.
- **The reply rides the per-webview message box**
  (`webview_ipc_reply`), embedded verbatim into the resolve script.
...
| `webview_bind_ipc(handle, ipc)` | typed | wires the IPC: `ipc` is the JS-bound callback handle (`bffi_callback_bind`, signature `unit(str)`); validated against the contract, then invoked with `invoke_wait` from the loop thread |
...
until the JS side bound its ipc callback), the ipc handler and the
message box.
...
bun test wry               # skipped without the env
```
...
- **The ipc callback signature is fixed**: `unit(str)` (wire tags
  `[0, 5]`) over a `{ args: ["cstring"], returns: "void" }`
  JSCallback; `webview_bind_ipc` validates it and rejects anything
  else.
